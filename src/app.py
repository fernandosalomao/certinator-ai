...
async def _check_llm_endpoint() -> tuple[bool, str]:
    ...
    if _cfg.LLM_PROVIDER == "local":
        # FoundryLocal manages its own endpoint; liveness is implicit
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            # HEAD request to the base endpoint — we only care about
            # TCP connectivity + TLS handshake, not a valid API call.
...
def _check_thread_store() -> tuple[bool, str]:
    ...
    """
    return True, f"in-memory, threads={len(_thread_store)}"
