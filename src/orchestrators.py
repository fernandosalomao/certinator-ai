"""
Certinator AI — AG-UI Orchestrator Chain

Custom orchestrators that handle HITL request_info message filtering,
predict-state artifact cleanup, and message ordering fixes for the
CopilotKit ↔ MAF AG-UI bridge.

Orchestrator chain order:
  1. RequestInfoOrchestrator — filters messages when the workflow has
     pending request_info HITL calls.
  2. HumanInTheLoopOrchestrator — handles tool approval responses
     (accept / reject).  [from SDK, not defined here]
  3. PersistentDefaultOrchestrator — standard agent execution with
     in-memory thread persistence.
"""
...
from thread_store import get_or_create_thread
...
class RequestInfoOrchestrator(Orchestrator):
    ...
    **B) No pending requests** — a normal user message, but the
    ...
    async def _run_with_pending(
        ...
        # build MessagesSnapshotEvent payloads.
